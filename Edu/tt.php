...
$sql = "select qa_no from {$g5['quiz_answer_table']} where qa_end='N' and qa_uid='{$info['qa_uid']}' order by qa_no desc limit 1";
...
if( $qa_no ) {
	...
	if(sql_num_rows($result)) {
		...
		$kind_count = floor( 20 / count($kinds) );
		...
		$sql = "select qq_no from {$g5['qq_table']} where qq_quiz_code='{$quiz_code}' order by rand() limit 20";
		$result = sql_query($sql);
            ...
            <div class="is-con">
                <div class="fl">
					...
					if( $count_question ) {
						...
						## 문제은행 정보조인
						$sql = "select * from {$g5['qq_table']} where qq_quiz_code='{$quiz_code}' and qq_no='$qq_no'";
						$result = sql_query($sql);
						
						$quiz_question_info = sql_fetch($result);
					...
					?>
                    <div id="question_tab_<?=$seq?>" style="display: ;">
						<a name="#qq_<?=$seq?>">
