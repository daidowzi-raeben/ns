...
// AJAX helper to get chapter info by lesson and folder name
if (isset($_POST['action']) && $_POST['action'] == 'get_chapter_info') {
    $l_no = (int)$_POST['lesson'];
    $cpt_folder = trim($_POST['cpt_folder']); // e.g. "02", "03"
    
    // Find chapter matching the folder name (c_url)
    $sql = " SELECT c.cpt_no, c.cpt_contents 
             FROM {$g5['chapter_table']} c
             JOIN {$g5['contents_table']} cn ON c.cpt_contents = cn.c_no
             WHERE c.cpt_lesson = '{$l_no}' AND cn.c_url = '{$cpt_folder}' ";
    $row = sql_fetch($sql);
    ...
    if ($row) {
        $rtn = array(
            "res" => true,
            "cpt_no" => $row['cpt_no'],
            "c_no" => $row['cpt_contents']
        );
    } else {
        $rtn = array(
            "res" => false,
            "msg" => "차시 정보를 찾을 수 없습니다."
        );
    }
