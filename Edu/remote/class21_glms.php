...
?>
        ...
        <div class="progress-wrap">
            <div class="progress-info">
                <p class="fl"><span>진도율</span>
                    <?php echo get_int2num($userLessonData['app_study_rate'])?> %
                </p>
...
?>
                <tr>
                    ...
                    <td style="text-align:left;">
                        <?php echo get_chapterName($row['cpt_contents'])?>
                    </td>
                    <td>
                        <?= substr($result['lssn_sdate'], 0, 10)?>
                    </td>
                    <td>
                        <?= substr($result['lssn_edate'], 0, 10)?>
                    </td>
                    <td>
                        <?php echo $att_rate_text?>
                    </td>
                    <td>
                        <?php
    if ($cpt_study_rate[$i] >= 100 || $userLessonData['app_study_rate'] == 100) {
?>
                        <a href="#" onclick="$('.new__pop').show();" class="cg-btn"><span class="enterClass2"
                                lno="<?php echo $lssn_no?>" cno="<?php echo $row['cpt_no']?>">학습완료</span></a>
                        <?php
    }
    elseif ($cpt_study_rate[$i - 1] >= 100 || $i == 1) {
?>
                        <a href="#" class="cg-btn active"><span class="enterClass2" lno="<?php echo $lssn_no?>"
                                cno="<?php echo $row['cpt_no']?>">학습하기</span></a>
                        <?php
    }?>
                    </td>
                </tr>
