...
if ($w == '')
{
    sql_query(" insert into {$g5['compliance_table']} set pc_register_id = '{$member['mb_id']}', pc_register_day = '".G5_TIME_YMDHIS."', pc_register_ip = '{$_SERVER['REMOTE_ADDR']}', {$sql_common} ");
}
else if ($w == 'u')
{
    ...
    $sql = " update {$g5['compliance_table']}
                set {$sql_common}
                where pc_no = '{$pc_no}' ";
    sql_query($sql);
}
...
goto_url('./process_cp_list.php?'.$qstr.'&amp;', false);
?>