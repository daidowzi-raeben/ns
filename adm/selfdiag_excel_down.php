...
for($i=1; $res=sql_fetch_array($qry); $i++)
{
	...
	$sql2 = " select * from {$g5['point_table']} where mb_id = '{$res['mb_id']}' and po_rel_table = 'self1' and po_year = '$bl_year' and po_semi = '$bl_cate' limit 0, 1 ";
	$row2 = sql_fetch($sql2);
	$sql2 = " select * from {$g5['point_table']} where mb_id = '{$res['mb_id']}' and po_rel_table = 'self1' and po_year = '$bl_year' and po_semi = '$bl_cate' limit 1, 1 ";
	$row3 = sql_fetch($sql3);
...
$filename = "윤리실천자가진단".date("ymd", time()).".xls";
