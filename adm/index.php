...
//오늘 학습회원 수
$sql = " select count(distinct mb_id) as cnt from {$g5['point_table']} where po_datetime >= curdate() ";
$row = sql_fetch($sql);
$today_lrn_count = $row['cnt'];

//어제 학습회원 수
$sql = " select count(distinct mb_id) as cnt from {$g5['point_table']} where po_datetime > CURDATE() - INTERVAL 1 DAY and po_datetime <= curdate() ";
$row = sql_fetch($sql);
