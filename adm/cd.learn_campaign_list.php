...
<!-- 				<th scope="col" id="mb_list_id" rowspan="2">마일리지 합계</th> -->
			...
			for ($i=0; $row=sql_fetch_array($result); $i++) {
				...
				$sql_list = "SELECT * from sj_lms_adm_list ORDER BY idx asc";
				$result_list = sql_query($sql_list);
				for ($k=0; $row2=sql_fetch_array($result_list); $k++) {
					$lssn_data = sql_fetch("SELECT * from sj_lms_lesson AS a WHERE a.lssn_class = '".$row2['idx']."' ");
            ...
            <tr class="<?php echo $bg; ?>">
				...
				for ($u=0; $row3=sql_fetch_array($result_list); $u++) { 
					$sql_is = " SELECT * from sj_lms_lesson_result WHERE mb_id = '".$mb_id."' AND lssn_no = '".$row3['lssn_no']."' limit 1 ";
