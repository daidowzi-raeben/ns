...
if ($w == '')
{
	...
	*/
	#echo " insert into {$g5['member_table']} set mb_id = '{$mb_id}', mb_password = '".get_encrypt_string($mb_password)."', mb_datetime = '".G5_TIME_YMDHIS."', mb_ip = '{$_SERVER['REMOTE_ADDR']}', mb_email_certify = '".G5_TIME_YMDHIS."', bl_no = '4', {$sql_common} ";
	#exit();
    sql_query(" insert into {$g5['member_table']} set mb_id = '{$mb_id}', mb_password = '".get_encrypt_string($mb_password)."', mb_datetime = '".G5_TIME_YMDHIS."', mb_ip = '{$_SERVER['REMOTE_ADDR']}', mb_email_certify = '".G5_TIME_YMDHIS."', bl_no = '4', {$sql_common} ");
}
else if ($w == 'u')
{
    $mb = get_member($mb_id);
...
if($_POST['type'] != "adm")
	...
	goto_url('./admin_list.php?'.$qstr.'&amp;', false);
?>