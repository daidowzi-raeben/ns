...
<div class="local_ov02">
	...
	<div class="r_div">
		...
		<a href="./selfdiag_excel_down.php?sfl=<?php echo $sfl ?>&amp;str=1&amp;bl_year=<?php echo $bl_year?>&amp;bl_cate=<?php echo $bl_cate?>" target="_blank" id="member_add" class="btn btn_04">EXCEL</a>
		<?php } ?>
	</div>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        #윤리실천 자가진단 1, 2차 포인트 내역 가져오기
        $sql2 = " select * from {$g5['point_table']} where mb_id = '{$row['mb_id']}' and po_rel_table = 'self1' and po_year = '$bl_year' and po_semi = '$bl_cate' limit 0, 1 ";
		$row2 = sql_fetch($sql2);
		$sql3 = " select * from {$g5['point_table']} where mb_id = '{$row['mb_id']}' and po_rel_table = 'self1' and po_year = '$bl_year' and po_semi = '$bl_cate' limit 1, 1 ";
		$row3 = sql_fetch($sql3);
